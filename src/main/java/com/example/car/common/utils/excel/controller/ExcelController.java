...
public class ExcelController {
    ...
    public static final String[] VALID_FILE_TYPE = {"xlsx", "zip", "xls"};

    public static final String file = "/home/img/excel/";
...
//    public void ExcelOnline(String deptid, String status, String startTime, String endTime) throws
...
//                            temp = endTime;
//                        } else {
//                            down += DateUtil.datetime(temp, eChatBean.getCreateDate(), DateUtil
...
//        List<DeviceAlarmSeverity> deviceAlarmSeverities = deviceAlarmSeverityMapper.selectAlarmExcel(startTime,
//                endTime, deptid);
...
//        exportExcelUtil.exportExcel("在线时间", columnNames, deviceAlarmSeverities,
//                new FileOutputStream("D:/alarm/" + deptid + ".xls"),
//                ExportExcelUtil.EXCEL_FILE_2003);
//    }

    @RequestMapping("ExcelMuck")
    public void ExcelMuck(String name, Integer size, Integer index, String BeginTime, String pname,
                          String type, String time1, String time2, String car, HttpServletResponse response) {
        List<MuckExcel> list;
        if (StringUtils.isEmpty(car)) {
            list = this.muckMapper.selectMuckExcel(name, size, index * size, pname, BeginTime,
                    time1, time2, type);
            for (MuckExcel objectMap : list) {
                List<String> strings = muckMapper.selectCarByPermitNo(objectMap.getPermitno());
                objectMap.setCar(strings);
            }
        } else {
            list = this.muckMapper.selectMuckExcel1(name, size, index * size, pname, BeginTime,
                    type, time1, time2, car);
        ...
        ExcelKit.$Export(MuckExcel.class, response).downXlsx(list, false);
    }
}
